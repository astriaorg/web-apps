...
export function TxnDetails({inputOne, inputTwo, expectedOutputFormatted, priceImpact, minimumReceived}: TxnDetailsProps) {
  const { tokenOneSymbol, tokenTwoSymbol, tokenTwoValue, oneToOneLoading, error, setFlipDirection, flipDirection } = useOneToOneQuote(inputOne, inputTwo);
    ...
    <div className="flex flex-col items-center justify-center h-full mt-20">
      ...
      <div className="text-white font-medium mt-6 text-center">
        ...
        <div className="flex items-center gap-1 justify-center text-sm">
          <span>
            {formatDecimalValues(inputOne?.value || "0", 6)} <span>{inputOne?.token?.coinDenom}</span>
          </span>
          <span>
            for
          </span>
...
export function SwapTxnSteps({ swapPairs, txnStatus, setTxnStatus }: SwapTxnStepsProps) {
  const inputOne = swapPairs[0]?.inputToken
  const inputTwo = swapPairs[1]?.inputToken
  const { expectedOutputFormatted, priceImpact, minimumReceived } = useTxnInfo({ swapPairs })

  return (
    <div className="h-[320px]">
      {txnStatus === TXN_STATUS.IDLE && (
        <TxnDetails inputOne={inputOne} inputTwo={inputTwo} expectedOutputFormatted={expectedOutputFormatted} priceImpact={priceImpact} minimumReceived={minimumReceived} />
      )}
      {txnStatus === TXN_STATUS.PENDING && (
        <TxnLoader inputOne={inputOne} inputTwo={inputTwo} expectedOutputFormatted={expectedOutputFormatted} />
      )}
      {txnStatus === TXN_STATUS.SUCCESS && (
        <TxnSuccess inputOne={inputOne} inputTwo={inputTwo} expectedOutputFormatted={expectedOutputFormatted} />
      )}
      {txnStatus === TXN_STATUS.FAILED && (
        <TxnFailed inputOne={inputOne} inputTwo={inputTwo} expectedOutputFormatted={expectedOutputFormatted} />
      )}
    </div>
  );
}
